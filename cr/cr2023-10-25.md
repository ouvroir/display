...
Note : nous aurions pu simplement déclarer `bot:Element` comme sous-classe de `display:Exhibit`, mais la définition de `bot:Element` ne permet pas la spécialisation de l’expôt en tant que `bot:Element`.
